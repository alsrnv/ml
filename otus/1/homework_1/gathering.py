...
def gather_process():
    ...
    for car_model in MODELS:
        initial_url = BASE_URL + car_model + page_part + '1'
        total_pages = scrapper.get_total_pages(scrapper.get_html(initial_url))
        for i in range(1, total_pages + 1):
            time.sleep(5)
            url_gen = BASE_URL + car_model + page_part + str(i)
            logger.info('processing ' + str(i) + '  ' + url_gen )
            df = scrapper.get_page_data(scrapper.get_html(url_gen))
            res = pd.concat([res, df])
    res.to_csv('data.csv', index = False) 


def convert_data_to_table_format():
    """Лишняя функция, так как в gather_process() уже данные в csv"""
...
def stats_of_data():
    ...
    df['car_name_'] = df['car_name']
    m = df.groupby('car_name').agg({'car_year':'median', 
                                'car_price':'median',
                                'car_distance':'median',
                                'car_name_':'count'                          
                               }).reset_index().rename(columns = {'car_name_':'car_cnt'})
    m['all_cnt'] = m['car_cnt'].sum()
    m['percent'] = np.round(100*m['car_cnt']/m['all_cnt'], 2)
    m.drop('all_cnt', axis = 1, inplace = True)

    dict_names = {'car_name':'Модель', 
                  'car_year':'Год(Медиана)',
                  'car_price': 'Цена(Медиана)', 
                  'car_distance':'Пробег(Медиана)', 
                  'car_cnt':'Количество объявлений', 
                  'percent':'Доля от объявлений(%)'}
    m.rename(columns = dict_names, inplace = True)

    print('Статистика по моделям автомобилей Toyota')
    print(m.sort_values('Количество объявлений', ascending = False).reset_index().drop('index', axis = 1))
